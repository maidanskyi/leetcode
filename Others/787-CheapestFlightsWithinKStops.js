...
const findCheapestPrice = (n, flights, src, dst, K) => {
  ...
  for (const [start, end, price] of flights) {
    if (!flightMap[start]) flightMap[start] = {};
    flightMap[start][end] = price;
  }

  if (!flightMap[src]) return -1;

  const queue = [[src, 0, -1]];

  while (queue.length) {

    const [location, price, stop] = queue.shift();
    if (stop > K) continue;
    if (location === dst) return price;

    for (const [flightTo, cost] of Object.entries(flightMap[location])) {
      queue.push([Number(flightTo), price + cost, stop + 1]);
    }

    queue.sort(([l1, p1, s1], [l2, p2, s2]) => p1 - p2);
...
console.log(findCheapestPrice(
  ...
  [
    ...
    [0,8,57],[6,14,68],[16,8,3],[13,0,65],[2,13,6],[5,13,5],[8,11,31],[6,10,20],
    [6,2,33],[9,1,3],[14,9,58],[12,3,19],[11,2,74],[12,14,48],[16,11,100],
    [3,12,38],[12,13,77],[10,9,99],[15,13,98],[15,12,71],[1,4,28],[7,0,83],
    ...
    [5,16,64],[4,5,65],[15,14,70],[12,0,13],[16,14,52],[3,10,80],[14,11,85],
    [15,2,77],[4,11,19],[2,7,49],[10,7,78],[14,6,84],[13,7,50],[11,6,75],
    [5,10,46],[13,8,43],[9,10,49],[7,12,64],[0,10,76],[5,9,77],[8,3,28],
    [11,9,28],[12,16,87],[12,6,24],[9,15,94],[5,7,77],[4,10,18],[7,2,11],
    [9,5,41]
  ],
  13,
  4,
